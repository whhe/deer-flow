...
8. **ToolProgressMiddleware** - State-machine-based stagnation guard (RFC #3177, optional, if `tool_progress.enabled`). Outer wrapper around ToolErrorHandlingMiddleware so it receives results already stamped with `deerflow_tool_meta`. Tracks per-(thread, tool) consecutive "no-new-info" calls (error, partial_success, or Jaccard-near-duplicate success results). Three transition paths: (a) `recoverable_by_model=True` (no_results, not_found, permission, Jaccard-duplicate success): ACTIVE → WARNED (terminal — hint re-injected on each subsequent problem; model is expected to change strategy, so BLOCKED is never reached); (b) `recoverable_by_model=False, action≠stop` (rate_limited, transient): ACTIVE → WARNED → BLOCKED after `warn_escalation_count` more problems (model cannot fix these by retrying); (c) `recoverable_by_model=False, action=stop` (auth, config, internal): immediately BLOCKED on the first occurrence, skipping WARNED. **Division of labor with LoopDetectionMiddleware (item 19):** ToolProgressMiddleware is a result-quality guard — fires after tool execution and blocks specific tools that stop producing new information. LoopDetectionMiddleware is a call-pattern guard — fires after the model responds (before execution) and hard-stops the whole turn when the model repeatedly issues identical tool_calls. They are complementary: ToolProgressMiddleware is fine-grained (per-tool BLOCK, other tools unaffected), LoopDetectionMiddleware is coarse-grained (strips all tool_calls). Both can inject HumanMessage hints in the same model call without conflict; neither reads the other's internal state.
9. **ToolErrorHandlingMiddleware** - Converts tool exceptions into error `ToolMessage`s so the run can continue instead of aborting; stamps every result with `deerflow_tool_meta` (status / error_type / recoverable_by_model / recommended_next_action / source) via `tool_result_meta.normalize_tool_result`
